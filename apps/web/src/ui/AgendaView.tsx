...
export const AgendaView: React.FC<AgendaViewProps> = ({
	...
	const overdueItems = todayItems.filter(
		(i) => i.heading.deadline && isPast(parseDate(i.heading.deadline)!),
	);
	const rescheduleItems = todayItems.filter(
		(i) => i.heading.scheduled && isPast(parseDate(i.heading.scheduled)!),
	);
	...
	const importantItems = activeTodos.filter(
		(i) => i.heading.priority === "A" || i.heading.tags?.includes("Important"),
	);
	const nextActionItems = activeTodos.filter(
		(i) =>
			!importantItems.includes(i) &&
			(i.heading.priority === "B" || getCategory(i) === "Next Action"),
	);
	const shoppingItems = activeTodos.filter(
		(i) =>
			!importantItems.includes(i) &&
			!nextActionItems.includes(i) &&
			getCategory(i) === "Shopping",
	);
	const inboxItems = activeTodos.filter(
		(i) =>
			!importantItems.includes(i) &&
			!nextActionItems.includes(i) &&
			!shoppingItems.includes(i) &&
			getCategory(i) === "Inbox",
	);
	const otherItems = activeTodos.filter(
		(i) =>
			!importantItems.includes(i) &&
			!nextActionItems.includes(i) &&
			!shoppingItems.includes(i) &&
		...
		return (
			...
			>
				<div className="flex-1 flex items-start p-1">
					{showFile && (
						<span className="text-xs font-mono text-gray-400 mt-1 w-20 flex-shrink-0 truncate mr-2">
							{getCategory(item)}
						</span>
					)}
					...
					<div className="flex-1">
						<div className="flex items-center flex-wrap">
							{item.heading.todoKeyword && (
								<span
									className={`text-xs font-bold mr-2 px-1 rounded ${
										item.heading.todoKeyword === "TODO"
											? "bg-red-100 text-red-700"
											: item.heading.todoKeyword === "NEXT"
												? "bg-blue-100 text-blue-700"
												: item.heading.todoKeyword === "WAITING"
							...
							{item.heading.priority && (
								<span className="text-yellow-600 font-mono text-sm mr-1 font-bold">
									[#{item.heading.priority}]
								</span>
							)}
	...
	return (
		<div className="max-w-4xl mx-auto p-6 bg-white shadow-sm min-h-screen">
			...
			{/* 2. Agenda (Today) */}
			<div className="mb-8">
				<h3 className="text-lg font-bold mb-2 text-gray-700 border-b border-gray-200 pb-1">
				...
				<div className="space-y-4">
					...
					{rescheduleItems.length > 0 && (
						<div>
							<h4 className="text-sm font-semibold text-orange-600 mb-1">
								Reschedule
							</h4>
							<ul className="space-y-1">
			...
			{waitingItems.length > 0 && (
				<div className="mb-8">
					<h3 className="text-lg font-bold mb-2 text-yellow-700 border-b border-yellow-200 pb-1">
						WAITING
					</h3>
					<ul className="space-y-1">
