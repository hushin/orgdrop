...
export class FileCache {
	...
	async getFile(
		path: string,
		knownRev?: string,
	): Promise<{ content: string; rev: string; cached: boolean }> {
		const dropboxPath = path.startsWith("/") ? path : `/${path}`;
		const key = `${this.tokenHash}:${dropboxPath}`;

		const cached = (await this.kv.get(key, { type: "json" })) as {
			rev: string;
			content: string;
		} | null;
		...
		if (cached) {
			...
			if (knownRev) {
				if (cached.rev !== knownRev) {
					console.log(
						`[FileCache] Stale (cached: ${cached.rev}, known: ${knownRev})`,
					);
	...
	private async revalidate(path: string, cachedRev: string) {
		try {
			...
			if (metadata.rev !== cachedRev) {
				console.log(
					`[FileCache] Updating ${path} (cached: ${cachedRev}, current: ${metadata.rev})`,
				);
