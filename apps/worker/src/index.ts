...
app.get("/api/search", async (c) => {
	...
	const query =
		c.req.query("q")?.toLowerCase().replace(/\s+/g, " ").trim() || "";
	...
	try {
		...
		for (let i = 0; i < searchFiles.length; i++) {
			...
			try {
				const { content, cached } = await fileCache.getFile(
					file.path,
					file.rev,
				);
		...
		console.log(
			`[Search] Cache misses: ${cacheMissCount}/${searchFiles.length}`,
		);
...
app.get("/api/config", async (c) => {
	...
	try {
		const client = new DropboxClient(token);
		const configPath =
			rootPath === "/" ? "/orgdrop.json" : `${rootPath}/orgdrop.json`;

		let config = {
			agendaPaths: ["*.org", "areas/", "projects/", "resources/"],
		};
		try {
			...
			if (e.message && e.message.includes("path/not_found")) {
				console.log(
					"Config file (orgdrop.json) not found, using default configuration.",
				);
			} else {
		...
		await Promise.all(
			agendaFiles.map(async (file) => {
				try {
					...
					const { content, rev: fetchedRev } = await fileCache.getFile(
						file,
						rev,
					);
					...
					// Check Agenda Cache
					const cachedAgenda = (await c.env.DROPBOX_CACHE.get(agendaCacheKey, {
						type: "json",
					})) as { rev: string; items: AgendaItem[] } | null;
					...
					if (cachedAgenda && cachedAgenda.rev === fetchedRev) {
						...
						c.executionCtx.waitUntil(
							c.env.DROPBOX_CACHE.put(
								agendaCacheKey,
								JSON.stringify({ rev: fetchedRev, items: fileItems }),
							),
						);
...
function filterFiles(
	files: string[],
	appConfig: { rootPath: string; config: { agendaPaths: string[] } },
): string[] {
	...
	return files.filter((file) => {
		return config.agendaPaths.some((agendaPath) => {
			...
			const dirPrefix = resolvedPath.endsWith("/")
				? resolvedPath
				: `${resolvedPath}/`;
...
function extractTasks(nodes: any[], file: string, items: AgendaItem[]) {
	for (const node of nodes) {
		if (node.type === "heading") {
			...
			if (
				heading.todoKeyword &&
				heading.todoKeyword !== "DONE" &&
				heading.todoKeyword !== "CANCELED"
			) {
