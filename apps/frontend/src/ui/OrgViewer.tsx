...
export const OrgViewer: React.FC<OrgViewerProps> = ({
	file,
	resolveImage,
	onLinkClick,
}) => {
...
const HeadingRenderer: React.FC<HeadingRendererProps> = ({
	...
	return (
		<div
			className="group"
			id={node.properties?.["ID"] || node.properties?.["CUSTOM_ID"]}
			data-title={node.title}
		>
			<Tag className={`text-gray-900 ${sizeClasses} flex items-center`}>
				{node.todoKeyword && (
					<span
						className={`mr-2 px-2 py-0.5 rounded text-sm text-white ${
							node.todoKeyword === "TODO"
								? "bg-red-500"
								: node.todoKeyword === "DONE"
									? "bg-green-500"
									: node.todoKeyword === "NEXT"
										? "bg-blue-500"
										: "bg-gray-500"
						}`}
					>
				...
				{node.priority && (
					<span className="mr-2 text-yellow-600 font-mono">
						[#{node.priority}]
					</span>
				)}
				<span>
